...
10. 只用键盘走一遍：Tab 时每个按钮都有可见焦点环；口令弹窗打开后 Tab 不会跑到背景控件，Esc 关闭后焦点回到「同学信息」；底部面板关闭后焦点回到地图。宽屏（≥768px）下面板是居中的 560px 抽屉。
...
地图本身不能用键盘操作：它由 ECharts 画在 canvas 上，省份和城市无法被聚焦，因此只靠键盘没法下钻或查看某个地区的人数。页面其余部分已支持键盘——工具条按钮有可见焦点环，打开底部面板或口令弹窗时背景会 `inert`、Esc 可关闭、关闭后焦点归还给触发元素；关键配色的对比度由 `npm run build` 里的静态检查守住（`scripts/check-contrast.js`）。要补上地图这一环，需要改成 SVG 渲染，或者另外提供一份地区列表作为键盘入口。
