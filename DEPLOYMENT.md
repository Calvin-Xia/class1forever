...
只想调前端（地图渲染、下钻、缩放限制、口令弹窗、分享图）而不想连 Cloudflare 时，改用：

```bash
npm run dev:mock
```

它用一个本地服务器同时提供静态文件和假的 `/api/*`，口令默认是 `demo`。想让地图显示真实的聚合人数，可以按 `README.md` 里的说明先导出 `students:public:v1`，再用 `PUBLIC_DATA_FILE` 指过去。
...
5. 悬停省份时底部色带图例同步标出对应人数
6. 缩放和拖动被限制在可视范围内，地图不会被拖出画面
7. 分享按钮能生成包含地图、图例与统计数字的 PNG
